...
def _github_issue_payload() -> dict:
    issue = MagicMock()
...
def _gitlab_issue_payload() -> dict:
    issue = MagicMock()
    issue.external_id = "7"
    issue.key = "group/project#7"
    issue.title = "Fix login"
    issue.description = "Login 401"
    issue.status = "opened"
    issue.meta_data = {"url": "https://gitlab.example.com/group/project/-/issues/7"}
    issue.external_url = None
    project = MagicMock()
...
def test_preset_runner_payload_has_clone_fields():
    ...
    assert project["http_url_to_repo"] == "https://gitlab.example.com/group/project.git"
