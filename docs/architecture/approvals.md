...
**Repository context.** The permission hook records which repository a native tool call ran in. When the hook event includes a non-empty `cwd` inside a git work tree, the CLI runs `git rev-parse --show-toplevel` and `git remote get-url origin`, and computes cwd relative to that toplevel, with a hard 500 ms budget. Any error, including a timeout, omits the field (fail open). The result is sent as `repository` on `POST /api/v1/agents/permission-check` and stored as `_preloop_repository` inside `tool_args`, next to `_preloop_source`, so approval cards can show `owner/repo` (plus the relative path when it is not empty) without a schema migration. The session timeline shows the same marker when a tool-call activity's metadata carries it. Native-hook approvals store the marker on the approval request; they do not write a tool-call activity in this slice. A work tree with no `origin` remote is stored with an empty `remote` and `no_remote: true`. An `origin` that is not a `host/owner/repo` identity (a local path or `file://` remote) is also reported as `no_remote`. A directory that is not a git work tree stores nothing.

This is a trusted observation of the hook's cwd. It is not taken from tool arguments, and it is not a policy scope: native access rules, subject scope, and precedence do not consult it. Limits: a linked worktree reports that worktree's toplevel, not the main checkout. Only the `origin` remote is read when several remotes exist. Each string is bounded to 512 bytes. The remote is normalized to `host/owner/repo`: scheme, userinfo, credentials, a trailing `.git`, and a trailing slash are removed, and only the host is lowercased.
