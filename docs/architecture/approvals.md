...
**Repository context.** The permission hook records which repository a native tool call ran in. When the hook event includes a non-empty `cwd` inside a git work tree, the CLI runs `git rev-parse --show-toplevel` and `git remote get-url origin`, and computes cwd relative to that toplevel, with a hard 500 ms budget. Any error, including a timeout, omits the field (fail open). The result is sent as `repository` on `POST /api/v1/agents/permission-check` and stored as `_preloop_repository` inside `tool_args`, next to `_preloop_source`, so approval cards and session timeline native-tool rows can show `owner/repo` (plus the relative path when it is not empty) without a schema migration. A work tree with no `origin` remote is stored with an empty `remote` and `no_remote: true`. A directory that is not a git work tree stores nothing.
